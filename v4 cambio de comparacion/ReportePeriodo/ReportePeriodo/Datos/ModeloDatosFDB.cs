...
{
    public enum tipoComandoFDB
    {
        query,
        storedProcedure
    }

    public class ModeloDatosFDB
    {
        private FbConnection Conexion = null;
        private FbCommand Comando = null;
        private FbDataAdapter Adapter = null;
        private FbTransaction Transaccion = null;
        private DataTable Tabla = null;
        private string cadenaConexion = string.Empty;


        //Constructor
        public ModeloDatosFDB(string cadena)
        {
            Configurar(cadena);
        }
        ...
        private void Configurar(string cadenaConexion)
        {
            try
            {
        ...
        public void Conectar()
        {
            ...
            try
            {
                if (this.Conexion == null)
                {
                    this.Conexion = new FbConnection();
        ...
        public void CrearComando(string sentenciaSQL, tipoComandoFDB tipo)
        {
            this.Comando = new FbCommand();
            this.Comando.Connection = Conexion;
            this.Comando.CommandTimeout = 0;
            if (tipo == tipoComandoFDB.query)
            {
        ...
        {
            FbParameter sqlPara = new FbParameter(nombre, FbDbType.Integer, 2);
            sqlPara.Value = valor;
        ...
        {
            FbParameter sqlPara = new FbParameter(nombre, FbDbType.VarChar, 20);
            sqlPara.Value = valor;
            sqlPara.Direction = direction;
            Comando.Parameters.Add(sqlPara);
        }
        ...
        {
            Adapter = new FbDataAdapter(Comando);
            Tabla = new DataTable();
            Adapter.Fill(Tabla);
            return Tabla;
        }

        public FbDataAdapter RegresaAdapter()
        {
            Adapter = new FbDataAdapter(Comando);
            return Adapter;
        }
