...
namespace ReportePeriodo.Modelo
{
    public class Model : IModel
    {
        ...
        private DatosLecheProd PromedioDatosLeche(DateTime fechaInicio, DateTime fechaFin, ref string mensaje)
        {
            DatosLecheProd response = new DatosLecheProd();
            ModeloDatosAccess db = new ModeloDatosAccess(conexionAccess);
            mensaje = string.Empty;

            try
            {
                string query = @"SELECT  IIF(SUM(IIF(VACASORDEÑA <> 0,1,0)) > 0 ,CLng(SUM(VACASORDEÑA)/SUM(IIF(VACASORDEÑA <> 0,1,0))) ,0) AS ORDEÑO
                                        ,IIF(SUM(IIF(LECPROD <> 0,1,0)) > 0 ,CLng(SUM(LECPROD)/SUM(IIF(LECPROD <> 0,1,0))),0)              AS LECHE
                                        ,IIF(SUM(IIF(ANTPROD <> 0,1,0)) > 0 ,CInt(SUM(ANTPROD)/SUM(IIF(ANTPROD <> 0,1,0))),0)              AS ANTIB
                                        ,IIF(SUM(IIF(TOTAL <> 0,1,0)) > 0 ,CLng(SUM(TOTAL)/SUM(IIF(TOTAL <> 0,1,0))),0)                    AS TOTALP
                                        ,IIF(IIF(SUM(IIF(VACASORDEÑA <> 0,1,0)) > 0 ,CLng(SUM(VACASORDEÑA)/SUM(IIF(VACASORDEÑA <> 0,1,0))) ,0) > 0,IIF(SUM(IIF(TOTAL <> 0,1,0)) > 0 ,CLng(SUM(TOTAL)/SUM(IIF(TOTAL <> 0,1,0))),0) / IIF(SUM(IIF(VACASORDEÑA <> 0,1,0)) > 0 ,CLng(SUM(VACASORDEÑA)/SUM(IIF(VACASORDEÑA <> 0,1,0))) ,0) ,0 ) AS MEDIA
                                        ,IIF(SUM(IIF(delord <> 0,1,0)) > 0 ,CInt(SUM(delord)/SUM(IIF(delord <> 0,1,0))),0)                 AS DEL
                                        ,IIF(SUM(IIF(VACAANTIB <> 0,1,0)) > 0 ,CInt(SUM(VACAANTIB)/SUM(IIF(VACAANTIB <> 0,1,0))),0)        AS ANT
                                FROM
                                (
	                                SELECT  m.FECHA
	                                        ,VACASORDEÑA
                ...
                db.AsignarParametro("@fechaInicio", ConvertToJulian(fechaInicio));
                db.AsignarParametro("@fechaFin", ConvertToJulian(fechaInicio));
                response = db.EjecutarConsultaTabla().AsEnumerable().Select( x => new DatosLecheProd()
                {
                    Año = fechaFin.Year,
                    Leche = x["LECHE"] != DBNull.Value ? Convert.ToDecimal(x["LECHE"]) : 0,
                    Antib = x["ANTIB"] != DBNull.Value ? Convert.ToDecimal(x["ANTIB"]) : 0,
                    Media = x["MEDIA"] != DBNull.Value ? Convert.ToDecimal(x["MEDIA"]) : 0,
                    Total = x["TOTALP"] != DBNull.Value ? Convert.ToDecimal(x["TOTALP"]) : 0,
                    Del = x["DEL"] != DBNull.Value ? Convert.ToDecimal(x["DEL"]) : 0,
                    Ant = x["ANT"] != DBNull.Value ? Convert.ToDecimal(x["ANT"]) : 0
                }).ToList().FirstOrDefault();
        ...
        //Mandar fechas del año pasado
        public DatosLecheProd MejorPromedio(DateTime fechaInicio, DateTime fechaFin, ref string mensaje)
        {
            DatosLecheProd response = new DatosLecheProd();
            try
            {
                List<DateTime> fechasMesAño = new List<DateTime>();
                List<DatosLecheProd> datos = new List<DatosLecheProd>();

                //llenar Lista de fechas
                for (int i = 0; i > -10; i--)
                {
                    DateTime fechaAux = fechaFin.AddYears(i);
                    fechasMesAño.Add(fechaAux);
                }

                //Lenar lista de datos
                foreach (DateTime fecha in fechasMesAño)
                {
                    DateTime fechaInicioAux = new DateTime(fecha.Year, fecha.Month, 1);
                    DatosLecheProd datoAux = PromedioDatosLeche(fechaInicioAux, fecha, ref mensaje);
                    datos.Add(datoAux);
                }


                //Comparar datos
                datos = datos.OrderByDescending(x => x.Media).ToList();

                response = datos.FirstOrDefault();
        ...
        public void DatoMejorPromedioUltimos10Años (Hoja1 promedio, DateTime fechaInicio, DateTime fechaFin, ref string mensaje)
        {
            mensaje = string.Empty;

            try
            {
                DatosLecheProd dato = MejorPromedio(fechaInicio, fechaFin, ref mensaje);

                if (dato.Media != null && dato.Media != 0)
                {
                    promedio.Leche = dato.Leche;
                    promedio.Antib = dato.Antib;
                    promedio.Media = dato.Media;
                    promedio.Total = dato.Total;
                    promedio.DEL = dato.Del;
                    promedio.ANT = dato.Ant;
                }
            }
            catch (Exception ex)
            {
