...
namespace ReportePeriodo.Modelo
{
    public class Modelo1 : IModelo1
    {
		...
		*/
		...
		public bool VerLitros(ref string mensaje)
		{
			bool response = false;
			ModeloDatosAccess bd = new ModeloDatosAccess();
			mensaje = string.Empty;
			...
			try
			{
				string query = @"SELECT rl.RANCHOLOCAL, r.VERLITROS
								 FROM RANCHOLOCAL rl
								 LEFT JOIN RANCHOS r ON rl.RANCHOLOCAL = r.CLAVE";
				...
				if (dt.Rows.Count > 0)
				{
					response = Convert.ToBoolean(dt.Rows[0]["VERLITROS"]);
				}
			}
			catch (DbException ex) { mensaje = ex.Message; }
			catch (Exception ex) { mensaje = ex.Message; }
		...
		public List<CalostroYOrdeña> ListaCalostroOrdeño(DateTime fechaInicio, DateTime fechaFin)
        {
			List<CalostroYOrdeña> response = new List<CalostroYOrdeña>();
			ModeloDatosAccess db = new ModeloDatosAccess();
			...
			try
            {
				...
				db.Conectar();
				db.CrearComando(query, tipoComandoDB.query);
		...
		public CalostroYOrdeña CalostroPromedio(DateTime fechaInicio, DateTime fechaFin)
        {
